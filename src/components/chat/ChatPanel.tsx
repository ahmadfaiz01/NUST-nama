...
export function ChatPanel({ compact = false }: { compact?: boolean }) {
    ...
    return (
        <div className="flex flex-col h-full min-h-0">
            <div className={`flex-1 min-h-0 overflow-y-auto space-y-4 ${compact ? "p-3" : "py-4"}`}>
                {turns.length === 0 && (
                        ...
                        <div className="flex flex-wrap gap-2">
                            {EXAMPLES.map((example) => (
                                <button
                                    key={example}
                                    onClick={() => ask(example)}
                                    className="border-2 border-nust-blue px-2 py-1 text-xs bg-white hover:bg-nust-orange/20"
                                >
                                    {example}
                                </button>
                            ))}
                ...
                {turns.map((turn, i) => (
                    <div key={i} className="space-y-2">
                        <p className="font-bold text-nust-blue break-words">{turn.question}</p>
                        ...
                        {turn.answer && (
                            <p
                                className={`text-sm whitespace-pre-wrap break-words ${turn.error ? "text-amber-700" : ""
                                    }`}
                            >
                                {turn.answer}
                            </p>
            ...
            <form
                ...
                className={`flex gap-2 border-t-2 border-nust-blue bg-white ${compact ? "p-3" : "pt-3"}`}
            >
                <input
                    value={question}
                    onChange={(e) => setQuestion(e.target.value)}
                    placeholder="Ask about NUST…"
                    className="flex-1 min-w-0 border-2 border-nust-blue px-3 py-2 text-sm outline-none focus:border-nust-orange"
                />
                <button
                    type="submit"
                    disabled={busy || !question.trim()}
                    className="btn bg-nust-orange text-nust-blue font-bold px-4 py-2 text-sm disabled:opacity-50"
                >
                    {busy ? "…" : "Ask"}
                </button>
            </form>
